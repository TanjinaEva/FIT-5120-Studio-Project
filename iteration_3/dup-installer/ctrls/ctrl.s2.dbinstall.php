...
class DUPX_DBInstall
{
    ...
    protected function dbConnect()
    {
        if (is_null($this->dbh)) {
            ...
            $this->supportedCollatesList = DUPX_DB::getSupportedCollateList($this->dbh);
	...
	public function runCleanupRotines()
    {
        ...
        DUPX_DB::mysqli_query($this->dbh, "DELETE FROM `".mysqli_real_escape_string($this->dbh, $GLOBALS['DUPX_AC']->wp_tableprefix)."duplicator_pro_packages`");
        ...
        foreach ($opts_delete as $value) {
            DUPX_DB::mysqli_query($this->dbh, "DELETE FROM `".mysqli_real_escape_string($this->dbh, $GLOBALS['DUPX_AC']->wp_tableprefix)."options` WHERE `option_name` = '".mysqli_real_escape_string($this->dbh, $value)."'");
        }
	...
	*/
    ...
    public function writeLog()
    {
        ...
        if ($result = DUPX_DB::mysqli_query($this->dbh, "SHOW TABLES")) {
    ...
    private function applyQueryCharsetCollationFallback($query) {
        if ($this->dbcharsetfb && !empty($this->dbcharsetfbval) && preg_match('/ CHARSET=([^\s;]+)/i', $query, $charsetMatch)) {
            ...
            if (false === array_search($charset, $this->supportedCharSetList)) {
                DUPX_Log::info("LEGACY CHARSET FALLBACK: ".$charset." to ".$this->dbcharsetfbval);
        ...
        if ($this->dbcollatefb && !empty($this->dbcollatefbval) && preg_match('/ COLLATE=([^\s]+)/i', $query, $collateMatch)) {
            $collate = $collateMatch[1];
            if (false === array_search($collate, $this->supportedCollateList)) {
                DUPX_Log::info("LEGACY COLLATION FALLBACK: ".$collate." to ".$this->dbcollatefbval);
