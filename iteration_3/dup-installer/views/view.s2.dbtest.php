...
DUPX.testDBConnect = function ()
{
    ...
    var formData = { };
    $.each($('#s2-input-form').serializeArray(), function() {
        formData[this.name] = this.value;
    });
    
    var ajaxData = $.extend({}, <?php
            echo DupProSnapJsonU::wp_json_encode_pprint(array(
                DUPX_Ctrl_ajax::AJAX_NAME   => true,
                DUPX_Ctrl_ajax::ACTION_NAME => DUPX_Ctrl_ajax::ACTION_DATABASE_CHECK,
                DUPX_Ctrl_ajax::TOKEN_NAME  => DUPX_Ctrl_ajax::generateToken(DUPX_Ctrl_ajax::ACTION_DATABASE_CHECK)
            ));
            ?>, formData);
...
DUPX.intTestDBResults = function(data, result)
{
    ...
    if (collateValInputObj && typeof  data.payload.extra.collate !== 'undefined') {
        ...
        for (i = 0; i < data.payload.extra.collate.list.length; i++) {
            ...
            if (typeof data.payload.extra.charset.selected == undefined || data.payload.extra.charset.selected == '') {
