...
class DUPX_View_S1
{
    ...
    public static function getReq($ret_is_dir_writable)
    {
        ...
        $req['40'] = (DUPX_Custom_Host_Manager::getInstance()->isManaged() && $GLOBALS['DUPX_AC']->wp_tableprefix != DUPX_WPConfig::getValueFromLocalWpConfig('table_prefix', 'variable')) ? 'Fail' : 'Pass';
    ...
    public static function options()
    {
        ...
        <!-- ==========================
        OPTIONS -->
        <div class="hdr-sub1 toggle-hdr <?php echo $opened ? 'close' : 'open'; ?>" data-type="toggle" data-target="#s3-adv-opts">
            <a href="javascript:void(0)"><i class="fa fa-plus-square"></i>Options</a>
        </div>
        <!-- START TABS -->
        <div id="s3-adv-opts" class="hdr-sub1-area tabs-area <?php echo $opened ? '' : 'no-display'; ?>">
            <div class="tabs">
                <ul>
    ...
    public static function otherUrlsAndPaths()
    {
        ...
        <div id="other-path-url-options" class="dupx-opts dupx-advopts">
            <small><i>These values are not recommended for change.  Please use caution when updating.</i></small>
            <?php
