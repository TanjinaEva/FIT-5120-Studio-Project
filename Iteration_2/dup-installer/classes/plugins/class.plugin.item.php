...
class DUPX_Plugin_item
{
    ...
    public function isActive($subsite = -1)
    {
        if ($this->data['active'] === true) {
            return true;
        } else if ($subsite === -1 && !empty($this->data['active'])) {
            return true;
        } else if ($this->data['active'] === true || (is_array($this->data['active']) && in_array($subsite, $this->data['active']))) {
            return true;
        } else {
            return false;
        }
    ...
    {
        return in_array($this->data['slug'], DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_IGNORE_PLUGINS));
    }

    public function isForceDisabled()
    {
        return in_array($this->data['slug'], DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_FORCE_DIABLE_PLUGINS));
    }
    ...
    public function setActivationAction($subsite = -1, $networkCheck = false, $forceActivation = false)
    {
        ...
        if ($forceActivation) {
            DUPX_Log::info('PLUGINS ['.__FUNCTION__.']: set forced activation action '.DUPX_log::varToString($this->slug), DUPX_Log::LV_DEBUG);
        ...
        if ($networkCheck) {
            ...
            if ($this->isInactive($subsite) || $this->isNetworkActive()) {
                $activate = true;
            }
        }

        if ($activate || $this->data['deactivateAction']) {
            DUPX_Log::info('PLUGINS ['.__FUNCTION__.']: set activation action '.DUPX_log::varToString($this->slug), DUPX_Log::LV_DEBUG);
    ...
    public function setDeactivateAction($subsite = -1, $shortMsg = null, $longMsg = null, $networkCheck = false)
    {
        ...
        if ($deactivate) {
            DUPX_Log::info('PLUGINS ['.__FUNCTION__.']: set deactivate action '.DUPX_log::varToString($this->slug), DUPX_Log::LV_DEBUG);
    ...
    public function getPluginPath()
    {
        $mainDir = false;
        if ($this->isMustUse()) {
            if (($mainDir = DUPX_Plugins_Manager::getMuPluginsPath()) === false) {
                $mainDir = false;
            }
        } else if ($this->isDropIns()) {
            if (($mainDir = DUPX_Plugins_Manager::getWpContentPath()) === false) {
                $mainDir = false;
            }
        } else {
            if (($mainDir = DUPX_Plugins_Manager::getPluginsPath()) === false) {
                $mainDir = false;
            }
        }
    ...
    public function deactivate()
    {
        ...
        DUPX_Log::info('[PLUGINS MANAGER] deactivate '.DUPX_Log::varToString($this->slug), DUPX_Log::LV_DETAILED);

        $deactivated     = false;
        $orgiFileManager = DUPX_Orig_File_Manager::getInstance();

        if ($this->isMustUse() || $this->isDropIns()) {
            if (($pluginPath = $this->getPluginPath()) == false) {
                DUPX_Log::info('PLUGINS: can\'t remove plugin '.$this->slug.' because don\'t exists');
            } else {
                $orgiFileManager->addEntry($this->slug, $pluginPath, DUPX_Orig_File_Manager::MODE_MOVE);
                $deactivated = true;
            }
        } else {
    ...
    public function getOrgiStatus($subsiteId)
    {
        ...
        } else if ($this->isActive($subsiteId)) {
    ...
    public function uninstall()
    {
        ...
        try {
            ...
            DUPX_Log::info("UNINSTALL PLUGIN ".DUPX_Log::varToString($this->data['slug']), DUPX_Log::LV_DEBUG);
                ...
                if ($already_defined_uninstall_const) {
                    ...
                    foreach ($prohibited_codes as $prohibited_code) {
                        if (false !== stripos($uninstall_file_content, $prohibited_code)) {
                            DUPX_Log::info("Can't include uninstall.php file of the ".$this->data['slug']." because prohibited code found");
                            return false;
                        }
                    }
                }
    ...
    public function __get($key)
    {
        if (!isset($this->data[$key])) {
            throw new Exception('prop '.$key.' don\'t exists');
        }
