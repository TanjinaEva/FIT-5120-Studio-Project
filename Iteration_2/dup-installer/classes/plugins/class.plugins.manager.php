...
final class DUPX_Plugins_Manager
{
    ...
    private function setCustomPluginsActions()
    {
        ...
        $longMsg = "It is deactivated by default  automatically. <strong>You must reactivate from the WordPress admin panel after completing the installation</strong> "
            ...
            ." After activating it, your site's frontend will look good.";
    ...
    public static function getWpContentPath()
    {
        ...
        if (is_null($path)) {
            $path = false;
            $safePath = DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_PATH_CONTENT_NEW);
    ...
    public function getDefaultActivePluginsList($subsiteId = -1)
    {
        ...
        foreach ($this->plugins as $plugin) {
            if ($networkInstall) {
                if ($plugin->isNetworkActive($subsiteId) || $plugin->isMustUse() || $plugin->isDropIns()) {
                    $result[] = $plugin->slug;
                }
            } else {
    ...
    public static function getPluginsPath()
    {
        ...
        if (is_null($path)) {
            ...
            if (($pPath = $GLOBALS['DUPX_AC']->getDefineValue('PLUGINDIR', false)) !== false) {
                $safePath = DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_PATH_NEW).'/'.$pPath;
                if (!is_dir($safePath) || !is_readable($safePath)) {
    ...
    public function setAutoActions($subsiteId = -1)
    {
        ...
        if ($paramManager->getValue(DUPX_Paramas_Manager::PARAM_SITE_URL_OLD) != $paramManager->getValue(DUPX_Paramas_Manager::PARAM_SITE_URL) &&
