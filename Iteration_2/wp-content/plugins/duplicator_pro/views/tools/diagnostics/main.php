...
if (empty($archive_file)) {
    ...
    if (file_exists($root_path.'/installer.php')) {
        $installer_file_path = $root_path.'/installer.php';
    }
    if (file_exists($root_path.'/dpro-importinstaller.php')) {
        $installer_file_path = $root_path.'/dpro-importinstaller.php';
    }
    if (!empty($installer_file_path)) {
        $installer_file_data = file_get_contents($installer_file_path);
        if (preg_match('/const[\s\t]+ARCHIVE_FILENAME[\t\s]*?=[\t\s]*?[\'"](.+)[\'"]/', $installer_file_data, $match)) {
            $temp_archive_file      = sanitize_text_field($match[1]);
            $temp_archive_file_path = $root_path.'/'.$temp_archive_file;
...
$archive_path        = empty($archive_file) ? '' : $root_path . '/' .$archive_file;
$long_installer_path = (isset($_GET['in'])) ? $root_path . '/' . sanitize_text_field($_GET['in']) : '';
