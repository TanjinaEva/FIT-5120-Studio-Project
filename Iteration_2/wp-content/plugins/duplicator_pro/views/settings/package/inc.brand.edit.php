...
<form id="dpro-package-brand-form" action="<?php echo esc_url($brand_edit_url); ?>" method="post" data-parsley-ui-enabled="true">
	...
	<?php if ($_REQUEST['action'] == 'default') : ?>
		<table class="provider form-table">
			...
			<tr>
				<th scope="row"><label><?php DUP_PRO_U::esc_html_e("Active"); ?></label></th>
	...
	<?php else: ?>
		<table class="provider form-table">
			...
			<tr>
				<th scope="row"><label for="brand-active"><?php DUP_PRO_U::esc_html_e("Active"); ?></label></th>
				<td>
                    <input type="checkbox" name="active" id="brand-active" value="true"<?php echo $brand->active ? ' checked' : '' ; ?> />
					<label for="brand-active"><?php DUP_PRO_U::esc_html_e("Make this the active brand"); ?></label>
				</td>
			</tr>
		</table>
	<?php endif; ?>
    ...
    <br style="clear:both" />
    <button class="button button-primary" type="button"<?php echo ($_REQUEST['action'] == 'default') ? ' disabled' : ' onclick="DupPro.Settings.Brand.Save()"'; ?>><?php DUP_PRO_U::esc_html_e('Save Brand'); ?></button>
</form>
