...
class DUP_PRO_Installer
{
    ...
    function __construct($package)
    {
        ...
        if (($wpConfigPath = self::getWpConfigFilePath()) !== false) {
    ...
    private function create_archive_config_file()
    {
        ...
        DUP_PRO_Log::infoTrace('DB INFO IN CREATE ARCHIV CONFIG');
        DUP_PRO_Log::infoTrace(print_r($ac->dbInfo, true));
        ...
        $ac->wproot               = duplicator_pro_get_root_path();
        ...
        $ac->is_outer_root_wp_config_file = (!file_exists(duplicator_pro_get_root_path().'/wp-config.php')) ? true : false;
    ...
    private function getWpInfo()
    {
        ...
        $wpInfo->configs->realValues->homePath       = get_home_path();
        $wpInfo->configs->realValues->contentUrl     = content_url();
        $wpInfo->configs->realValues->contentPath    = WP_CONTENT_DIR;
        $updDirs                                     = wp_upload_dir();
        $wpInfo->configs->realValues->uploadBasePath = $updDirs['basedir'];
    ...
    /**
     * get wp-config path of false if don't exists
     * 
     * @return boolean|string
     */
    private static function getWpConfigFilePath()
    {
        $root_path = duplicator_pro_get_root_path();
        if (file_exists($root_path.'/wp-config.php')) {
            /** The config file resides in ABSPATH */
            return $root_path.'/wp-config.php';
        } else if (@file_exists(dirname($root_path).'/wp-config.php') && !@file_exists(dirname(ABSPATH).'/wp-settings.php')) {
            /** The config file resides one level above ABSPATH but is not part of another installation */
            return dirname($root_path).'/wp-config.php';
        } else {
            return false;
        }
    }
    ...
    private function add_extra_files($package)
    {
        ...
        $file_list_filepath      = DUP_PRO_U::safePath(DUPLICATOR_PRO_SSDIR_PATH_TMP).'/'.$this->Package->NameHash.DUP_PRO_Archive::FILES_LIST_FILE_NAME_SUFFIX;
        $dir_list_filepath       = DUP_PRO_U::safePath(DUPLICATOR_PRO_SSDIR_PATH_TMP).'/'.$this->Package->NameHash.DUP_PRO_Archive::DIRS_LIST_FILE_NAME_SUFFIX;
    ...
    private function add_extra_files_using_duparchive($installer_filepath, $scan_filepath, $file_list_filepath, $dir_list_filepath, $sql_filepath, $archive_filepath, $archive_config_filepath, $wpconfig_filepath)
    {
        ...
        try {
            $htaccess_filepath = duplicator_pro_get_root_path().'/.htaccess';
