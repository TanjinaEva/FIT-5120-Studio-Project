...
class DUP_PRO_Server 
{
    ...
    public static function getRequirments() {
        ...
        $handle_test = @opendir($root_path);
        // Removing is_writable on root - 
        //$dup_tests['IO']['WPROOT']	= is_writeable(duplicator_pro_get_root_path()) && $handle_test ? 'Pass' : 'Fail';
    ...
    public static function getChecks($package) {
         ...
         */
        $files['wp-config.php'] = file_exists(DUP_PRO_U::safePath($root_path . '/wp-config.php'));
    ...
    public static function getInstallerFiles() {
        ...
        $root_path = duplicator_pro_get_root_path();
        return array(
            DUPLICATOR_PRO_INSTALL_PHP => $root_path . '/' . $installer_base_name,
            $installer_backup_filename => $root_path . '/' .  $installer_backup_filename,
            ...
            'dup-wp-config-arc__[HASH].txt' => $root_path . '/dup-wp-config-arc__*.txt',          
			'wp-config.duporig'		=> $root_path . '/wp-config.duporig',

            DUPLICATOR_PRO_ORIG_FOLDER_PREFIX . '[HASH]' . DUP_PRO_U::__('(directory)') => $root_path . '/' . DUPLICATOR_PRO_ORIG_FOLDER_PREFIX . DUPLICATOR_PRO_INSTALLER_HASH_PATTERN,
            'dup-installer-data__[HASH].sql' => $root_path . '/dup-installer-data__'.DUPLICATOR_PRO_INSTALLER_HASH_PATTERN.'.sql',
            'dup-database__[HASH].sql' => $root_path . '/dup-database__'.DUPLICATOR_PRO_INSTALLER_HASH_PATTERN.'.sql',
            'dup-installer-log_[HASH].txt' => $root_path . '/dup-installer-log__'.DUPLICATOR_PRO_INSTALLER_HASH_PATTERN.'.txt',
            'dup-installer-bootlog__[HASH].txt' => $root_path . '/' . 'dup-installer-bootlog__'.DUPLICATOR_PRO_INSTALLER_HASH_PATTERN.'.txt',
            'duplicator_pro_params_overwrite.php'=> $root_path . '/duplicator_pro_params_overwrite.php',
        );
    }
