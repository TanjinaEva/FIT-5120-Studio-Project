...
class DUP_PRO_CTRL_Tools extends DUP_PRO_CTRL_Base
{
    ...
    public function runScanValidator($post)
    {
        ...
        try {
            //CONTROLLER LOGIC
            $path = isset($post['scan-path']) ? $post['scan-path'] : duplicator_pro_get_root_path();
            if (!is_dir($path)) {
    ...
    public function prepareArchiveForImport($post)
    {
        ...
        try {
            ...
            if(isset($post['archive-filename'])) {
                ...
                $newArchiveFilepath = $root_path . '/' . basename($archive_filepath);

                if(!file_exists($root_path . '/' . $post['archive-filename']))
                {
	            ...
	            //$extracted_installer_filepath = duplicator_pro_get_root_path() . '/installer-backup.php';
				$extracted_installer_filepath = duplicator_pro_get_root_path() . "/{$installer_name}";
                ...
                if($fileExt == 'zip') {
                 ...
                 // RSR TODO: for now always use ziparchive   $useShellZip = ($global->get_auto_zip_mode() == DUP_PRO_Archive_Build_Mode::Shell_Exec);
                    ...
                    DUP_PRO_Zip_U::extractFiles($newArchiveFilepath, $relativeFilepaths, $root_path, $useShellZip);
                    ...
                    DupArchiveEngine::expandFiles($newArchiveFilepath, $relativeFilepaths, $root_path);
                ...
                //$final_installer_filepath= $root_path . 'installer-'
