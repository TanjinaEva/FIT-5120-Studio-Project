...
class DUPX_DB
{
     ...
     * @param bool $reset           if true reinit tables list
     *
     * @return bool  Does the table name exist in the database
     */
    public static function tableExists($dbh, $table_name, $reset = false)
    {
        static $tablesList = null;

        if (is_null($tablesList) || $reset) {
            // RSR TODO: retrieve full list of tables
            $tablesList = self::queryColumnToArray($dbh, "SHOW TABLES");
        }
     ...
     * @return type
     */
    public static function mysqli_query($link, $sql, $logFailLevel = DUPX_Log::LV_DEFAULT)
    {
        if (($result = mysqli_query($link, $sql)) === false) {
            if (DUPX_Log::isLevel($logFailLevel)) {
                ...
                $file    = $callers[0]['file'];
                $line    = $callers[0]['line'];
                $sqlLog  = substr($sql, 0, DUPX_Log::isLevel(DUPX_Log::LV_DEBUG) ? 10000 : 500);
            ...
            if (DUPX_Log::isLevel(DUPX_Log::LV_HARD_DEBUG)) {
                ...
                $file    = $callers[0]['file'];
                $line    = $callers[0]['line'];
                DUPX_Log::info('DB QUERY ['.$file.':'.$line.']: '.DUPX_Log::varToString(substr($sql, 0, 2000)), DUPX_Log::LV_HARD_DEBUG);
            }
        }
        return $result;
    }
