...
class DUPX_S_R_ITEM
{
    ...
    const TYPE_PATH_UNTRAILINGSLASHIT = 'path_un';
    ...
    public function __construct($search, $replace, $type = DUPX_S_R_ITEM::TYPE_STRING, $prority = 10, $scope = array())
    {
        ...
        switch ($type) {
            ...
            case DUPX_S_R_ITEM::TYPE_PATH:
                $this->search  = DupProSnapLibIOU::safePathTrailingslashit($search);
                $this->replace = DupProSnapLibIOU::safePathTrailingslashit($replace);
                break;
            case DUPX_S_R_ITEM::TYPE_PATH_UNTRAILINGSLASHIT:
                $this->search  = DupProSnapLibIOU::safePathUntrailingslashit($search);
                $this->replace = DupProSnapLibIOU::safePathUntrailingslashit($replace);
                break;
    ...
    public function getPairsSearchReplace()
    {
        switch ($this->type) {
            ...
            case self::TYPE_PATH:
            case self::TYPE_PATH_UNTRAILINGSLASHIT:
    ...
    public static function searchReplacePath($search_path, $replace_path)
    {
        ...
        /**
         * the trim is necessary because in windows there are paths with mixed separators
         * for example: C:\xampp\domains\temptest1.winloc/wp-content
         * 
         * @todo this is a temporary fix waiting for the engine to support replacements with regex
         * 
         */
        $search_path_unsetSafe = rtrim(DUPX_U::unsetSafePath($search_path), '/\\');
        $trimReplace           = rtrim($replace_path, '/\\');

        return array_merge($result, self::searchReplaceWithEncodings($search_path_unsetSafe, $trimReplace));
    }
